...
// ─── MONTE CARLO TAB ──────────────────────────────────────────
export function MonteCarloTab({ result }: Props) {
  const mc = result.monte_carlo;
  const initial = result.backtest?.equity_curve?.[0] ?? 10000;

  if (!mc) {
    return (
      <div className="p-4 text-[#4a6080] text-xs">
        {result.backtest?.num_trades === 0
          ? "No trades to simulate"
          : "Monte Carlo unavailable (insufficient equity curve data)"}
      </div>
    );
  }

  const profitColor = (mc.prob_profit ?? 0) >= 60 ? "text-[#00ff88]" : (mc.prob_profit ?? 0) >= 50 ? "text-[#ffa502]" : "text-[#ff4757]";
  const confidenceColor = mc.confidence === "HIGH" ? "text-[#00ff88]" : "text-[#ffa502]";

  // Simple probability bar
  const probPct = mc.prob_profit ?? 0;

  return (
    <div className="p-3 space-y-4">
      {/* Prob profit gauge */}
      <div>
        <div className="text-[#4a6080] text-xs mb-1">PROBABILITY OF PROFIT ({mc.runs.toLocaleString()} runs)</div>
        <div className={`text-3xl font-bold mb-2 ${profitColor}`}>{probPct.toFixed(1)}%</div>
        <div className="h-2 bg-[#1e2d4a] rounded">
          <div className="h-2 rounded transition-all" style={{
            width: `${probPct}%`,
            background: probPct >= 60 ? "#00ff88" : probPct >= 50 ? "#ffa502" : "#ff4757"
          }} />
        </div>
      ...
      {/* Equity outcomes */}
      <div>
        <div className="text-[#4a6080] text-xs font-bold mb-1">EQUITY OUTCOMES</div>
        <div className="grid grid-cols-3 gap-2 text-xs">
          {[
            { label: "WORST", val: mc.worst_equity, color: "text-[#ff4757]" },
            { label: "MEDIAN", val: mc.median_equity, color: "text-[#ffa502]" },
            { label: "BEST", val: mc.best_equity, color: "text-[#00ff88]" },
          ].map((m) => (
            <div key={m.label} className="bg-[#0a0e1a] border border-[#1e2d4a] p-2 rounded text-center">
              <div className="text-[#4a6080] mb-1">{m.label}</div>
              <div className={m.color}>${m.val?.toFixed(0)}</div>
              <div className="text-[#4a6080] text-xs">
                {((((m.val ?? initial) - initial) / initial) * 100).toFixed(1)}%
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Risk metrics */}
      <div>
        <div className="text-[#4a6080] text-xs font-bold mb-1">RISK METRICS</div>
        <Row label="Avg Max Drawdown" value={`${mc.avg_max_dd?.toFixed(1)}%`} color="text-[#ff4757]" />
        <Row label="VaR (5%)" value={`${mc.var_5?.toFixed(1)}%`} color="text-[#ff4757]" />
      </div>

      {/* Method */}
      <div>
        <div className="text-[#4a6080] text-xs font-bold mb-1">SIMULATION</div>
        <Row label="Method" value={mc.method} />
        <Row label="Confidence" value={mc.confidence} color={confidenceColor} />
        {mc.divergence !== null && (
          <Row label="Method Divergence" value={`${mc.divergence}%`}
            color={mc.methods_agree ? "text-[#00ff88]" : "text-[#ffa502]"} />
        )}
        <Row label="Methods Agree" value={mc.methods_agree ? "YES" : "NO"}
          color={mc.methods_agree ? "text-[#00ff88]" : "text-[#ffa502]"} />
      </div>
    </div>
  );
