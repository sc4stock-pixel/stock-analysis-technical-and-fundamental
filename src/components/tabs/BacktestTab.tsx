...
export default function BacktestTab({ result }: Props) {
  ...
  return (
    <div className="p-3 space-y-3">
        ...
        <div className="border border-[#1e2d4a] rounded bg-[#080d1a] p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className={`text-xs px-2 py-0.5 rounded border font-mono ${
              cmp.winner === "score" ? "border-[#00ff88] text-[#00ff88] bg-[#00ff88]/10"
              : cmp.winner === "supertrend" ? "border-[#ffa502] text-[#ffa502] bg-[#ffa502]/10"
            ...
            <div>
              ...
              {[...cmp.score.trades].reverse().slice(0, 4).map(t => (
                <div key={t.trade_num} className="flex justify-between text-[0.6rem] font-mono mb-0.5">
                  <span className="text-[#4a6080]">{fmtDate(t.entry_date)}</span>
                ...
                {dir === 1 ? (
                  <>
                    ...
                    {openRet !== null && openRet !== undefined && (
                      <span className={openRet >= 0 ? "text-[#00ff88]" : "text-[#ffa502]"}>{openRet >= 0 ? "+" : ""}{openRet.toFixed(1)}%</span>
                    )}
                    {openRet !== null && openRet !== undefined && <span className="text-[#4a6080]">·</span>}
      ...
      {eqData.length > 2 && (
        <div>
          <div className="text-[#4a6080] text-xs mb-1">EQUITY CURVE
            {cmp && <span className={`ml-2 px-1.5 py-0.5 rounded text-[0.6rem] font-mono border ${bt.signal_bars === 0 ? "border-[#ffa502]/40 text-[#ffa502]" : "border-[#00d4ff]/40 text-[#00d4ff]"}`}>
          ...
          <div className="h-28">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={eqData}>
                ...
                <Line type="monotone" dataKey="v" stroke={bt.signal_bars === 0 ? "#ffa502" : "#00d4ff"} strokeWidth={1.5} dot={false} />
      ...
      {waterfallData.length > 0 && (
        <div>
          <div className="text-[#4a6080] text-xs mb-1">{bt.signal_bars === 0 ? "ST" : "SCORE"} TRADE RETURNS (last {waterfallData.length})</div>
          <div className="h-20">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={waterfallData} barSize={12}>
                ...
                <Bar dataKey="r">{waterfallData.map((d, i) => <Cell key={i} fill={d.fill} />)}</Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-x-4">
        <div>
          ...
          <Row label="Total Return" value={`${bt.total_return?.toFixed(1)}%`} color={(bt.total_return ?? 0) >= 0 ? "text-[#00ff88]" : "text-[#ff4757]"} />
        ...
        <div>
          ...
          <Row label="Win Rate" value={`${bt.win_rate?.toFixed(1)}%`} color={(bt.win_rate ?? 0) >= 55 ? "text-[#00ff88]" : "text-[#ff4757]"} />
          <Row label="Expectancy" value={`${bt.expectancy?.toFixed(2)}%`} color={(bt.expectancy ?? 0) >= 0 ? "text-[#00ff88]" : "text-[#ff4757]"} />
          <Row label="Profit Factor" value={bt.profit_factor?.toFixed(2) ?? "—"} color={(bt.profit_factor ?? 0) >= 1.5 ? "text-[#00ff88]" : "text-[#ffa502]"} />
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-2 text-xs">
          {[{ label: "AVG", val: bt.avg_duration?.toFixed(1) }, { label: "MED", val: bt.median_duration?.toFixed(0) },
            { label: "WIN AVG", val: bt.avg_winner_duration?.toFixed(1) }, { label: "LOSE AVG", val: bt.avg_loser_duration?.toFixed(1) }]
            .map((m) => (
