...
type ColKey =
  ...
  | "signal" | "st_status" | "sepa" | "tfm_10d" | "rsi" | "macd_hist"
              ...
              return (
                ...
                >
                  ...
                  <td className="px-2 py-1.5 text-center">
                    {(() => {
                      const s = r.sepa_metadata;
                      if (!s) return <span className="text-[#2a3d5a] text-[0.6rem]">--</span>;
                      const pip = (active: boolean | null, label: string) => {
                        if (active === null) return (
                          <span key={label} title={`${label}: N/A`}
                            className="inline-block px-1 py-0.5 rounded text-[0.6rem] border mx-0.5 text-[#2a3d5a] border-[#2a3d5a]/20 opacity-30">
                            —
                          </span>
                        );
                        return (
                          <span key={label} title={label}
                            className={`inline-block px-1 py-0.5 rounded text-[0.6rem] font-bold border mx-0.5
                              ${active
