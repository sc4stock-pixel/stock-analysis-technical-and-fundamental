...
// ── 2. Southbound Flow ────────────────────────────────────────
// Primary:   Fetch static JSON generated by Python (hosted on GitHub)
// Secondary: Yahoo ETF proxy (fallback)
async function getSouthboundFlow(): Promise<MacroFactor> {

  // ── Source A: Pre-generated JSON file ───────────────────────
  try {
    // Adjust the URL to your own repository and branch
    const jsonUrl = "https://raw.githubusercontent.com/sc4stock-pixel/stock-analysis-technical-and-fundamental.git/main/southbound_data.json";
    const res = await fetch(jsonUrl, {
      headers: { "User-Agent": "Mozilla/5.0" },
      cache: "no-store",   // or use next: { revalidate: 3600 } if you prefer ISR
    ...
    return {
      ...
      score,
      signal,
      detail: `Today ${todayStr}亿 CNY (static)`,
    };
  } catch (err) {
    console.error("[Southbound] GitHub JSON fetch failed, fallback to ETF proxy:", err);
