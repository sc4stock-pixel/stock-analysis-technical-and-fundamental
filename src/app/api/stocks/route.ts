...
async function fetchFundamentals(symbol: string): Promise<Fundamentals> {
  ...
  // Prioritize v7/quote for Vercel/Server-side stability
  const hosts = ["query1", "query2"];
  
  for (const host of hosts) {
    try {
      // Use v7 as primary attempt; it's much more reliable on cloud IPs
      ...
      const res = await fetch(url, {
        headers: {
          ...
          // Note: No Origin header here to avoid center-range flagging
        },
      ...
      // Map fields precisely based on Yahoo v7 keys
      const pe = q.trailingPE ?? null;
      const fpe = q.forwardPE ?? null;
      const epsT = q.epsTrailingTwelveMonths ?? q.trailingEps ?? null;
      const epsF = q.epsForward ?? q.forwardEps ?? null;
      
      // Calculate growth if not provided (common for HK stocks)
      let growth = q.earningsGrowth ?? null;
      if (growth === null && epsT && epsF && epsT !== 0) {
        growth = (epsF - epsT) / Math.abs(epsT);
      }

      return {
        pe_ratio: pe ? Math.round(pe * 10) / 10 : null,
        forward_pe: fpe ? Math.round(fpe * 10) / 10 : null,
        eps_trailing: epsT,
        eps_forward: epsF,
        eps_growth: growth ? Math.round(growth * 1000) / 10 : null,
        analyst_target: q.targetMeanPrice ? Math.round(q.targetMeanPrice * 100) / 100 : null,
        analyst_rating: q.recommendationKey ? 
          q.recommendationKey.charAt(0).toUpperCase() + q.recommendationKey.slice(1).toLowerCase() : null,
      };
    } catch (e) {
      console.error(`Fetch failed for ${host}:`, e);
      continue; 
    }
  }
  return empty;
